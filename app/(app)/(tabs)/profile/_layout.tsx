...
export default function ProfileLayout() {
  ...
  useEffect(() => {
    // 1. Get user ID from different sources
    const pathSegments = pathname.split('/');
    const pathUserId = pathSegments.includes('profile') && pathSegments[pathSegments.indexOf('profile') + 1];
    const paramUserId = userId;
    const currentUserId = authProfile?.id || session?.user?.id;
    
    // 2. Determine which user to show
    // Priority: URL param > path segment > current user
    const targetUserId = paramUserId || pathUserId || currentUserId;
    const isCurrentUserProfile = targetUserId === currentUserId;
    
    // 3. Fetch profile if needed and not already loaded
    if (targetUserId && targetUserId !== lastProcessedUserId) {
      fetchProfile(targetUserId, currentUserId);
      setLastProcessedUserId(targetUserId);
    }
    
    // 4. Clean up when navigating away from profile
    return () => {
  ...
  useEffect(() => {
    ...
    switch (currentScreen) {
      ...
      default:
        ...
        if (currentProfile) {
          if (currentProfile.id === (authProfile?.id || session?.user?.id)) {
